...
class DecisionTransformerCNN(nn.Module):
    def __init__(
        ...
        # 1) GPT-2 backbone (frozen positional embeddings)
        ...
        # 2) normalization buffers
        if state_mean is not None and state_std is not None:
            self.register_buffer("state_mean", state_mean.view(1,1,*state_shape).float())
            self.register_buffer("state_std",  state_std.view(1,1,*state_shape).float())
        else:
        ...
        with torch.no_grad():
            ...
            flat_size = conv(dummy).view(1, -1).size(1)
        self.cnn = nn.Sequential(
            conv,
            nn.Flatten(),
            nn.Linear(flat_size, self.hidden_size),
        )

        # 4) modality & timestep embeddings
        self.embed_timestep = nn.Embedding(max_length, self.hidden_size)
        self.embed_return   = nn.Sequential(
            nn.Linear(1, self.hidden_size),
            nn.Tanh(),
        )
        self.embed_action   = nn.Sequential(
            nn.Embedding(act_dim, self.hidden_size),
            nn.Tanh(),
        )
        ...
        self.predict_state  = nn.Linear(self.hidden_size, C*H*W)
        self.predict_action = nn.Linear(self.hidden_size, act_dim)  # logits
    ...
    def forward(self, states, actions, returns_to_go, timesteps, attention_mask):
        ...
        # 1) embed returns + position
        r_emb = self.embed_return(returns_to_go) + t_emb  # (B,T,H)

        # 2) embed states via CNN + position
        x_img = states.view(B*T, C, H, W)
        s_emb = self.cnn(x_img).view(B, T, self.hidden_size) + t_emb

        # 3) embed actions + position
        ...
        tok_mask = attention_mask.to(torch.bool).unsqueeze(1).repeat(1,3,1).view(B, 3*T)

        # transformer
        out = self.transformer(inputs_embeds=x,
                               attention_mask=tok_mask,
                               use_cache=False)
        h = out.last_hidden_state.view(B, T, 3, self.hidden_size).permute(0,2,1,3)
        #  h[:,0]=returns, h[:,1]=states, h[:,2]=actions

        return_preds = self.predict_return(h[:,2])
        state_preds  = self.predict_state(h[:,2])
        action_logits= self.predict_action(h[:,1])
        return state_preds, action_logits, return_preds

    @torch.no_grad()
    def get_action(self, states, actions, returns_to_go, timesteps):
        # similar reindexing & batching as before, mask = all ones
        B0, T0 = 1, states.shape[0]
        L = self.max_length
        if T0 > L:
            states, actions, returns_to_go, timesteps = [x[-L:] for x in (states,actions,returns_to_go,timesteps)]
        else:
            pad = L - T0
            pad_s = torch.zeros(pad, *states.shape[1:], device=states.device)
            pad_a = torch.zeros(pad, dtype=torch.long, device=actions.device)
            pad_r = torch.zeros(pad, 1, device=returns_to_go.device)
            pad_t = torch.zeros(pad, dtype=torch.long, device=timesteps.device)
            states = torch.cat([pad_s, states], dim=0)
            actions = torch.cat([pad_a, actions], dim=0)
            returns_to_go = torch.cat([pad_r, returns_to_go], dim=0)
            timesteps = torch.cat([pad_t, timesteps], dim=0)

        states        = states.unsqueeze(0)
        actions       = actions.unsqueeze(0)
        returns_to_go = returns_to_go.unsqueeze(0)
        timesteps     = torch.arange(L, device=states.device).unsqueeze(0)
        mask          = torch.ones(1, L, dtype=torch.bool, device=states.device)

        _, logits, _ = self.forward(states, actions, returns_to_go, timesteps, mask)
        a = logits[0,-1].argmax()
        return a
