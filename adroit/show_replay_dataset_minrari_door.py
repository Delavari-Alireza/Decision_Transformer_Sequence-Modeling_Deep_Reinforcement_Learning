...
# render_mode defaults to 'human', so switch to rgb_array
try:
    env = dataset.recover_environment(render_mode="rgb_array")
except TypeError:
    # fallback if recover_environment doesn't accept render_mode
    env = dataset.recover_environment()
obs_env, _ = env.reset()
...
for t in range(T):
    # step env with the expert action
    action = acts[t]
    ...
    # overlay text: timestep, reward sum, done
