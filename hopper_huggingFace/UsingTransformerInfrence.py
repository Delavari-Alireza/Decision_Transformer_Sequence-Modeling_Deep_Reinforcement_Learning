...
def get_action(model, states, actions, rewards, returns_to_go, timesteps):
    ...
    device = states.device  # get device of the input tensors

    attention_mask = torch.cat([torch.zeros(padding), torch.ones(states.shape[1])]).to(dtype=torch.long,
...
TARGET_RETURN = 10 / scale  # evaluation is conditioned on a return of 3600, scaled accordingly
